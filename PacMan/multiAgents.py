...
class MinimaxAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        def miniMaxValue(gameState, depth, agent):
          ##If agent is pacman, start the next depht
          if gameState.getNumAgents() == agent:
            return miniMaxValue(gameState, depth + 1, 0)

          ##Get evaluation for bottom stats
          if self.isTerminal(gameState, depth, agent):
            return self.evaluationFunction(gameState)
          
          ##Get optimal, both min and max, state of the successors
          succs = (
            miniMaxValue(gameState.generateSuccessor(agent, x), depth, agent + 1)
            for x in gameState.getLegalActions(agent)
          )
          ##Return either max or min
          return (max if self.isPacman(gameState, agent) else min)(succs)
      
        ##Return the best move
        return max(gameState.getLegalActions(0), key = lambda x: miniMaxValue(gameState.generateSuccessor(0, x), 0 , 1))
...
class AlphaBetaAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        def flow(state, depth, agent, A=float("-inf"), B=float("inf")):
            ...
            if self.isPacman(state, agent):
                return fetchValv(state, depth, agent, A, B, float('-inf'), max)
            else:
                return fetchValv(state, depth, agent, A, B, float('inf'), min)


        def fetchValv(state, depth, agent, A, B, ms, mf):
            bestScore = ms
            bestAction = None

            for action in state.getLegalActions(agent):
                successor = state.generateSuccessor(agent, action)
                score,_ = flow(successor, depth, agent + 1, A, B)
